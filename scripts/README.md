...
top, then a few lines that call the library under `src/csdrecon/`. Nothing in
...
CONFIG_NAMES = "ALL"          # every folder in data/
```
...
| `run_10_bundle_figures.py` | re-render the comparison gallery one bundle at a time — `figures/40_points_per_ray/` holds 4×40…8×40, and so on — so a plot carries five series instead of fifteen. Adds folders; the full gallery is left alone |
...
```
data/3_rays_40_points_500_samples/
...
once**, in `data/_device_pools/`, and reused. The number of rays
...
`src/csdrecon/config/capacitance_config.py` builds a new pool instead of silently
reusing the old one.
...
| what | file |
|---|---|
| the capacitance parameter space | `src/csdrecon/config/capacitance_config.py` |
| the train/test split, made once on device IDs | `src/csdrecon/study/device_split.py` |
| how a device is simulated | `src/csdrecon/simulation/device_factory.py` |
| how the rays are cut | `src/csdrecon/ml/ray_peaks.py` |
| the network | `src/csdrecon/ml/grid_model.py` |
| training constants (lr, batch size, loss, validation split) | `src/csdrecon/ml/grid_train.py` |
| the four stages themselves | `src/csdrecon/study/` |
| the per-device figures | `src/csdrecon/study/device_figures.py` |
| the shared figure house style | `src/csdrecon/config/figure_style.py` |
...
in `src/csdrecon/ml/grid_train.py`.
