...
Writes data/<rays>_rays_<points>_points_<n>_samples/ containing
...
The simulated devices are cached once in data/_device_pools/ and
...
from csdrecon.study import dataset
...
CONFIG = StudyConfig(
    ...
    n_rays=4,               # rays fired across the diagram
    n_points=40,            # points sampled along each ray

    # how much data
    n_train=300,            # devices the model trains on
    n_test=50,             # held-out devices, same distribution
    ...
    epochs=50,              # used by run_2, recorded here so all four agree
