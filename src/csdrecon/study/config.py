...
named after exactly those three numbers:
    ...
    data/3_rays_50_points_100_samples/
...
shared pool and every configuration reuses them:
    ...
    data/_device_pools/devices_n130_res100_c1df7b6bf/
...
class StudyConfig:
    ...
    @property
    def dir(self) -> str:
        # paths.CONFIG_ROOT, not data/, so run_0 can put a whole
        # sweep - datasets, models, evaluations and the comparison - inside
        # one folder under results/.  It still defaults to data/,
...
def _candidate_folders() -> List[str]:
    ...
    Looked for in the current CONFIG_ROOT, in data/ and in results/,
    ...
    # One folder per NAME: a budget left over in data/ from an
