...
def wipe(results=True, device_pools=False):
    """
    ...
    `device_pools` also throws away the simulated DEVICE POOLS, which is the
...
def run(template, rays, points, n_train, n_test, epochs,
        ...
        fresh_start=False, wipe_device_pools=False):
    ...
    t0 = time.time()
    if fresh_start:
        wipe(results=True, device_pools=wipe_device_pools)
